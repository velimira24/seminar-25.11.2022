...
После того, как в файле были совершены необходимые изменения надо нажать на сочетание клавиш *Ctrl +S*. Это сохранит изменения. Далее, чтобы измененныя в файле начали считываться в Git, надо набрать в терминале *git add.\название файла". Третим этапом будет непосредственное сохранение контрольной версии изменений в Git. Для этого в терминал вводится следующая команда *git commit -m "название изменений".
